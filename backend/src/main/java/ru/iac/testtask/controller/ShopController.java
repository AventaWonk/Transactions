...
public class ShopController {
    ...
    public List<Shop> searchByName(@RequestParam String query,
        ...
        return this.shopService.searchByAddress(query, offset, limit);
    }

    @GetMapping("/")
    public String index(@RequestParam(required = false, defaultValue = "10") int limit, Model model) {
        int shopCount = this.shopService.getShopCount();
        int totalPages = calculatePageCount(shopCount, limit);
    ...
    public String getPage(@PathVariable int page,
        ...
        int totalPages = calculatePageCount(shopCount, limit);
        int offset = calculateOffset(page, limit);
        final List<Shop> shopList = this.shopService.getAllShops(offset, limit);
